...
MASTER_FORW = "master_forf"
list_request_type.append(MASTER_FORW)
...
def send_request_to_nodes(remote_nodes, config_cluster, request_type, args):
    ...
    if len(remote_nodes) == 0:
        remote_nodes_addr = list(map(lambda x: x[0], get_remote_nodes(True, True)))
    else:
...
def distributed_api_request(request_type, agent_id, args=[], from_cluster=False):
    config_cluster = read_config()

    # Only current master responses distributed request.
    # The rest of masters send theirs requests to the current master
